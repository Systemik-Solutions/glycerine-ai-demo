...
async function decode(imageURL, boundingBox) {
    ...
    const polygon = data.result.segmentation[0].polygons[0];
    const segment = [];

    // Convert the 1-dimensional array `polygon` to a 2-dimensional array where each element is an array with x and y.
    for (let i = 0; i < polygon.length; i += 2) {
        segment.push([polygon[i] / ratio, polygon[i + 1] / ratio]);
    }
...
const segAllBtn = document.getElementById('segAll');
segAllBtn.addEventListener('click', async function() {
    const loader = document.getElementById("loaderPane");
    loader.style.display = "flex";
    ...
    for (const item of data.result) {
        const polygon = item.segment[0].polygons[0];
        const segment = [];
